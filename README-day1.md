...
## 1. import 하기
// 만약 다른 곳에서 가져오고 싶다면 Vue처럼 import를 사용해서 가져오면 된다.
// 단, 중괄호를 사용할 때가 있는데, 가져올 파일에서 export default가 아닌 단순 export function 등의 경우에 사용하면 된다.
...
## 3. React 컴포넌트는 항상 대문자로 시작해야 한다!!!!
## 4. React에서 CSS 사용하기
// 컴포넌트에 CSS를 넣기 위해서는 props를 사용한 후 최상위 컴포넌트에 해당 classsName을 넣던가,
function MyButton ({className, ...props}) {
  return (
    <button className={className} {...props}>i'm 버튼이에오</button>
  )
}
// function App() {
...
// 아니면 해당 컴포넌트에 바로 className을 넣으면 된다.
function MyButton () {
  return (
    <button className="button">i'm 버튼이에오</button>
  )
}
