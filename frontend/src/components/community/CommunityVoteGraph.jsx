  ...
  return (
    <div className={`vote-graph border ${className} inline-block`}>
      <div className="w-full h-10">
          <div className="flex min-w-[300px] h-10 rounded">
            <div
              className="h-full rounded bg-[#DC1B3E] min-w-[150px] hover:bg-red-600 cursor-pointer"
              style={{ width: `${data.rateMy}%` }}
              onClick={() => selectedVoteType(1)}
            >
              <span className="text-white text-xl p-1">{`게시자 과실: ${data.rateMy}%`}</span>
            </div>
            <div
              className="h-full rounded bg-white min-w-[150px] cursor-pointer"
              style={{ width: `${data.rateNeutral}%` }}
              onClick={() => selectedVoteType(3)}
            >
              <span className="text-black text-xl p-1">{`중립: ${data.rateNeutral}%`}</span>
            </div>
            <div
              className="h-full rounded bg-[#458EF7] min-w-[150px] cursor-pointer"
              style={{ width: `${data.rateYour}%` }}
          ...
          <button
            type="button" 
            className="flex flex-row w-[120px] h-[40px] hover:bg-slate-50 border-[0.5px] cursor-pointer"
            onClick={handleVote}>
