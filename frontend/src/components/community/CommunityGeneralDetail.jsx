...
function CommunityGeneralDetail() {
  ...
  const { id } = useParams();
  const getCommunityDetail = useCommunityStore(
    (state) => state.getCommunityDetail
  );
  ...
  const currentItems =
    data.comments.length > 0
      ? data.comments.slice(
          indexOfFirstItem,
          Math.min(data.comments.length, indexOfLastItem)
        )
      : [];
      ...
      <div className="grid grid-cols-12 min-h-[300px]">
        ...
        <div className="max-w-[1300px] col-span-8 place-content-center">
          <div className="h-auto border-x-2">
            ...
            <div className="flex border-box min-h-[34px] bg-[#BBBBBB]">
            ...
            <div className="min-h-[300px] p-3">
              ...
              <div
                className="min-h-[30px] font-normal"
            ...
            <div className="flow-root space-y-1 px-2 pb-3 pt-2">
              <CommunityGeneralCommentPost
                id={id}
                onCommentUpdate={handleCommentUpdate}
              />
              {currentItems.length > 0 ? (
                currentItems.map((comment, idx) => {
                  const commentDate = dayjs(comment.postedAt).format(
                    "YYYY-MM-DD HH:mm:ss"
                  );
                  return (
                    <div key={idx} className="flex flex-col h-auto border-b-2">
                      <div className="flex flex-row justify-between">
                        <div className="flex flex-row gap-3">
                          {isLawyer === true ? (
                            <img src={lawyerBadge} alt="" className="h-8 w-8" />
                          ) : (
                            <div className="h-8 w-8"></div>
                          )}
                          <div className="border-box text-xl">
                            {comment.userNickname}
                          </div>
                        </div>
                        <div className="flex flex-row gap-5">
                          <div className="justify-self-end col-span-3 text-xl">
                            {commentDate}
                          </div>
                          {storedUserId === comment.userNickname ? (
                            <CommentDeleteButton
                              ...
                              className="block hover:bg-gray-100 mr-5"
                            />
                          ) : (
                            <p></p>
                          )}
